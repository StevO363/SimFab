...
int main() {
  ...
  advectionKernel.insertNextLevelSet(substrate);
  {
    ...
    for (unsigned i = 0; i < numberOfEtchSteps; ++i) {
      ...
      lsVTKWriter<double>(mesh, "pattern/substrate-" + std::to_string(i) + ".vtk")
          .apply();
...

    {
      auto mesh = lsSmartPointer<lsMesh<>>::New();
      lsToSurfaceMesh<double, D>(substrate, mesh).apply();
      lsVTKWriter<double>(mesh, "pattern/substrate-" +
  ...
  // make disk mesh and output
  {
    ...
    lsVTKWriter<double>(mesh, lsFileFormatEnum::VTP, "pattern/diskMesh.vtk").apply();
  }
  ...
  auto fillLayer = lsSmartPointer<lsDomain<double, D>>::New(substrate);
  {
    ...
    for (unsigned i = 0; i < numberOfDepoSteps; ++i) {
      ...
      lsVTKWriter<double>(mesh, "pattern/fillLayer-" +
  ...
  // now output the final level sets
  {
    auto mesh = lsSmartPointer<lsMesh<>>::New();
    lsToSurfaceMesh<double, D>(substrate, mesh).apply();
    lsVTKWriter<double>(mesh, "pattern/final-substrate.vtk").apply();

    lsToSurfaceMesh<double, D>(fillLayer, mesh).apply();
    lsVTKWriter<double>(mesh, "pattern/final-fillLayer.vtk").apply();
  }
