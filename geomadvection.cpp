...
//   omp_set_num_threads(4);
...

  {
    ...
    lsVTKWriter<NumericType>(mesh, "geomAdvec/trench-0.vtk").apply();
  }
  ...
  for (NumericType time = 0; time < 4.;
    ...
    lsVTKWriter<NumericType>(mesh, "geomAdvec/trench-" + std::to_string(counter) + ".vtk")
        .apply();

    lsToMesh<NumericType, D>(newLayer, mesh).apply();
    lsVTKWriter<NumericType>(mesh, "geomAdvec/LS-" + std::to_string(counter) + ".vtk")
        .apply();
