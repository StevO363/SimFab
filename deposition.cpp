...
//   omp_set_num_threads(4);
...

  {
    ...
    lsVTKWriter<NumericType>(mesh, "deposit/trench-0.vtk").apply();
  }
  ...
  for (NumericType time = 0; time < 4.;
    ...
    lsVTKWriter<NumericType>(mesh, "deposit/trench-" + std::to_string(counter) + ".vtk")
        .apply();

    lsToMesh<NumericType, D>(newLayer, mesh).apply();
    lsVTKWriter<NumericType>(mesh, "deposit/LS-" + std::to_string(counter) + ".vtk")
        .apply();
